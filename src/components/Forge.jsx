          ...
          {nfts.map((nft) => (
            <div key={nft.id} className="border nft-card" data-nft-id={nft.id} data-nft-name={nft.name} onClick={() => handleMint(nft.id, nft.name)}>
              ...
              <div className="p-4">
                ...
                <button 
                  className="mt-4 w-full inline-flex justify-center border px-4 py-2 gradient-bg-button text-base text-black hover:from-blue-400 hover:to-purple-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" 
                  onClick={() => handleMint(nft.id, nft.name)}
                  disabled={!active} // disable button if user doesn't have a Web3-enabled wallet
                >
                  {active ? 'FORGE YOUR MEDAL' : 'CONNECT WALLET'}
                </button>
              </div>
            </div>
          ))}
